...
#define ADR_IDT			0x0026f800 // size = 0x00000800
#define ADR_GDT			0x00270000 // size = 0x00010000
#define MEMMAN_ADDR		0x003c0000 // size = 0x00008000
/* * * * asmhead.nas * * * */
struct BOOTINFO {
	char cyls;
	char leds;
	char vmode;			// 画面模式，已初始化为8
	char reserve;
	short scrnx, scrny;	// 画面分辨率，已初始化为(320,200)
	char* vram;			// 显示缓冲地址，已初始化为0x000a0000
};
/* * * * naskfunc.nas * * * */
void io_hlt(void);
void io_cli(void);
...
/* * * * memory.c * * * */
#define MEMMAN_FREES	4094  // sizeof(MEMMAN) = 32KB
